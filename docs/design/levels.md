...
## [FR-LVL-040] Level-scoped split factor
...
Source Level K feeds into target levels `(K - 1) × splitFactor + 1` through `K × splitFactor`.

Example — `sourceLevels = 2`, `targetLevels = 4` → `splitFactor = 2`:
- Source Level 1 → target Levels 1-2
- Source Level 2 → target Levels 3-4

## [FR-LVL-011] Cascading level constraint validation
...
Valid transitions from Phase N with 2 levels: 2, 4, 6, 8, … Invalid: 1, 3, 5, … A source phase with no levels (`null` or `0`) imposes no constraint on the next phase.

## [FR-LVL-050] Placeholder distribution across target levels
